...
def p1(func, y0, tspan, n_steps, method):
    ...
    if method == 'euler':
        ...
        for i in range(n_steps):
            k = np.zeros(4)
            k[0] = func(t[i], y[i])
            k[1] = func(t[i] + h/2, y[i] + h*k[0]/2)
            k[2] = func(t[i] + h/2, y[i] + h*k[1]/2)
            k[3] = func(t[i] + h, y[i] + h*k[2])
...
def p2():
    ...
    # given an interval length h, euler is the least accurate, followed by midpoint, and rk4 is the most accurate
    # 
    # 
    #
    #  
